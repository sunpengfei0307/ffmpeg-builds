...
# ==================== 可配置变量 ====================
ZLM_DIR="/data/sunpf/tools/ZLMediaKit"
...
SRTP_PREFIX="/data/sunpf/tools/deps/libsrtp2"
LIBSRTP_VER="v2.6.0"
# 运维台运行目录与默认落盘
ADMIN_ZLM="/data/sunpf/tools/zlm-admin/zlm-server"
...
build_libsrtp2() {
    ...
    local src="/data/sunpf/tools/deps/src/libsrtp"
    mkdir -p /data/sunpf/tools/deps/src
    if [[ ! -d "$src/.git" ]]; then
        rm -rf "$src"
...
install_to_admin() {
    ...
    if [[ ! -x "${src}/MediaServer" ]]; then
        echo "ERROR: 未找到 ${src}/MediaServer" >&2
        return 1
    fi
    cp -f "${src}/MediaServer" "${dst}/MediaServer"
    chmod +x "${dst}/MediaServer"
    cp -f "${src}/config.ini" "${dst}/config.ini"
    if [[ -f "${src}/default.pem" ]]; then
        cp -f "${src}/default.pem" "${dst}/default.pem"
    fi
    clean_unused_zlm_dirs "${ZLM_DATA_ROOT}" "${dst}/config.ini"
    echo "已安装 MediaServer + config.ini -> ${dst}"
...
echo "===== 步骤7：安装到 zlm-admin/zlm-server ====="
install_to_admin

echo "===== 编译完成 ====="
echo "二进制: ${OUT_DIR}/MediaServer"
echo "配置:   ${OUT_DIR}/config.ini"
echo "运维台副本: ${ADMIN_ZLM}/{MediaServer,config.ini}"
echo ""
echo "启动建议（96 核机器不要用默认 96 线程）:"
echo "  /data/sunpf/tools/zlm-admin/start.sh"
echo "  或: cd ${ADMIN_ZLM} && ./MediaServer -c ./config.ini -t 16"
