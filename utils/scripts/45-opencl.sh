...
ffbuild_build() {
    ...
    # FFmpeg 目标为 OpenCL 1.2；新版 Khronos 头未定义 CL_TARGET_OPENCL_VERSION 时
    # 会 #pragma message 并默认到 310，旧版 FFmpeg（如 6.1.x）编译噪声/兼容性差。
    ...
    echo "Cflags: -I\${includedir} -DCL_TARGET_OPENCL_VERSION=120" >> OpenCL.pc
...
# 即使未重编 OpenCL（FFMPEG_ONLY），也保证 FFmpeg --extra-cflags 带上目标版本
ffbuild_cflags() {
    echo -DCL_TARGET_OPENCL_VERSION=120
}
