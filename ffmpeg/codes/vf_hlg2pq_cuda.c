...
/* Embedded PTX from vf_hlg2pq_cuda.ptx.c (built from vf_hlg2pq_cuda.cu). */
extern const char vf_hlg2pq_cuda_ptx[];
...
static int hlg2pq_cuda_query_formats(AVFilterContext *ctx)
{
...
static av_cold int hlg2pq_cuda_load_functions(AVFilterContext *ctx)
{
    ...
    ret = ff_cuda_load_module(ctx, s->hwctx, &s->cu_module,
                              (const unsigned char *)vf_hlg2pq_cuda_ptx,
                              strlen(vf_hlg2pq_cuda_ptx));
    if (ret < 0)
        goto fail;
...
static int hlg2pq_cuda_filter_frame(AVFilterLink *link, AVFrame *src)
{
    ...
    if (src->color_primaries != AVCOL_PRI_BT2020 ||
        ...
        av_log(ctx, AV_LOG_ERROR,
               "Input must be BT.2020 primaries and BT.2020 non-constant luminance matrix\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
...
static const AVClass hlg2pq_cuda_class = {
    .class_name = "hlg2pq_cuda",
    .item_name  = av_default_item_name,
    .option     = hlg2pq_cuda_options,
...
AVFilter ff_vf_hlg2pq_cuda = {
    .name          = "hlg2pq_cuda",
    .description   = NULL_IF_CONFIG_SMALL("CUDA accelerated HLG to PQ HDR conversion"),
    .uninit        = hlg2pq_cuda_uninit,
    .query_formats = hlg2pq_cuda_query_formats,
    .priv_size     = sizeof(CUDAHlg2pqContext),
    .priv_class    = &hlg2pq_cuda_class,
    .inputs        = hlg2pq_cuda_inputs,
    .outputs       = hlg2pq_cuda_outputs,
