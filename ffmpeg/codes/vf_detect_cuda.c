...
static int detect_cuda_query_formats(AVFilterContext *ctx)
{
...
static av_cold int detect_cuda_load_functions(AVFilterContext *ctx)
{
    ...
    extern const char vf_detect_cuda_ptx[];
    ...
    ret = ff_cuda_load_module(ctx, s->hwctx, &s->cu_module,
                              (const unsigned char *)vf_detect_cuda_ptx,
                              strlen(vf_detect_cuda_ptx));
    if (ret < 0)
        goto fail;
...
static av_cold int alloc_gpu_buffers(AVFilterContext *ctx)
{
    ...
    s->host_boxes = av_mallocz_array(s->max_det, sizeof(*s->host_boxes));
    if (!s->host_boxes)
        return AVERROR(ENOMEM);
...
static int detect_cuda_filter_frame(AVFilterLink *link, AVFrame *src)
{
    ...
    ret = av_hwframe_get_buffer(outlink->hw_frames_ctx, dst, 0);
    if (ret < 0)
        goto fail;
...
trt_done:
    ...
    outlink->hw_frames_ctx = av_buffer_ref(s->frames_ctx);
...
static const AVOption detect_cuda_options[] = {
    ...
    { "model_type", "Primary model output format", OFFSET(model_type), AV_OPT_TYPE_INT, { .i64 = DETECT_MODEL_AUTO }, DETECT_MODEL_AUTO, DETECT_MODEL_YOLOV8_FACE, FLAGS, "model_type" },
    { "auto", "Detect format from engine tensors", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_AUTO }, 0, 0, FLAGS, "model_type" },
    { "yolov8", "YOLOv8 raw output [1,4+nc,anchors]", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_YOLOV8 }, 0, 0, FLAGS, "model_type" },
    { "yolov8_face", "YOLOv8-Face 3-head DFL output (yakhyo yolov8n-face.onnx)", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_YOLOV8_FACE }, 0, 0, FLAGS, "model_type" },
    { "face_model_type", "Face engine output format", OFFSET(face_model_type), AV_OPT_TYPE_INT, { .i64 = DETECT_MODEL_AUTO }, DETECT_MODEL_AUTO, DETECT_MODEL_YOLOV8_FACE, FLAGS, "face_model_type" },
    { "auto", "Detect format from face engine tensors", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_AUTO }, 0, 0, FLAGS, "face_model_type" },
    { "yolov8", "Face YOLOv8 engine", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_YOLOV8 }, 0, 0, FLAGS, "face_model_type" },
    { "yolov8_face", "Face YOLOv8-Face 3-head DFL engine", 0, AV_OPT_TYPE_CONST, { .i64 = DETECT_MODEL_YOLOV8_FACE }, 0, 0, FLAGS, "face_model_type" },
...
static const AVClass detect_cuda_class = {
    .class_name = "detect_cuda",
    .item_name  = av_default_item_name,
    .option     = detect_cuda_options,
...
AVFilter ff_vf_detect_cuda = {
    .name          = "detect_cuda",
    .description   = NULL_IF_CONFIG_SMALL("CUDA/TensorRT person and face detection with ROI side data"),
    .uninit        = detect_cuda_uninit,
    .query_formats = detect_cuda_query_formats,
    .priv_size     = sizeof(CUDADetectContext),
    .priv_class    = &detect_cuda_class,
    .inputs        = detect_cuda_inputs,
    .outputs       = detect_cuda_outputs,
