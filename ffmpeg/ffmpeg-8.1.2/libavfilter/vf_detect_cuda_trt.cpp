...
/* TRT 8+: destroy() is deprecated; objects from create* APIs are freed with delete. */
template<typename T>
struct TrtDestroy {
                                                  ...
                                                  char *errbuf, int errbuf_size)
{
    ...
    (NV_TENSORRT_MAJOR > 8 || (NV_TENSORRT_MAJOR == 8 && NV_TENSORRT_MINOR >= 5))
    build_cfg->setMemoryPoolLimit(MemoryPoolType::kWORKSPACE, 1ULL << 30);
#else
